...
e.g. A 0.50 BSQ fee payment tx could look like that:

 - Input 1: 10.000 BSQ
 - Input 2: 0.1 BTC
 - Output 1: 9.50 BSQ
 - Output 2: 0.09949500 BTC
 - Mining fee: 0.0005 (0.00049500 BTC + 0.00000500 BTC or 0.50 BSQ)

So in that case we only use 9.500 BSQ of the 10.000 BSQ from the input. As the second output is spending more than the remaining 0.50 BSQ it is invalid as a BSQ output and we consider it as a BTC output. The remaining 0.500 BSQ which was not used in the first output will be used for the mining fee, thus reduces the mining fee which is paid from the BTC input (input 2). With that model we can spend fees as small as 0.01 BSQ or 1 Bitcoin Satoshi.
...
Contributors can create a compensation request for the work they contributed to the project. This can be anything what has added value to the project. The contributors has no guarantee that their request gets accepted. So when they start working they need to be aware that there is no guarantee for a reward.
...
*A compensation request needs to contain following data*
 ...
 - BSQ Address
 - Onion address
 - Transaction ID (the compensation request transaction will become a valid issuance tx if request gets accepted after voting has completed)
 - Tx ID of fee payment tx
 - Contributor's Public key
 - Signature of compensation request (signature of hash of all request data excluding signature and tx ID)
 - Version

*Data structure of the OP_RETURN compensation request data*
 - 1 byte for type (0x01)
 - 1 byte for version (0x01)
 - 20 bytes for hash of payloadAsJson + signature (using Sha256Ripemd160)
...
e.g. A compensation request tx for requesting 5000 BSQ would look like that (fee is 1 BSQ):
 - Input 1: 30.00 BSQ (needed for fee payment)
 - Input 2: 0.1 BTC (needed for mining fee as well as we need 1 BTC output
 - Output 1: 29.00 BSQ (change output)
 - Output 2: 0.00005 BTC (Requested BSQ amount to BSQ address defined in request)
 - Output 3: 0.09946 BTC (optional BTC change output)
 - Output 4 (last): OP_RETURN data as defined above
 - Mining fee: 0.00050000 (0.00049000 BTC from input 2 + 0.00001000 BTC or 1 BSQ from input 1)

 The input 1 need to be larger than the fee so we enforce a BSQ change output (output 1). All outputs must not be smaller than the dust limit (2730 Satoshi).
 The BTC input at input 2 need to be at least the sum of the requested BSQ and the miner fee, in our case 0.00005000 BTC (requested BSQ) + 0.00049000 BTC miner fee.
...
Initially the voting is mainly for the compensation requests but there will be some flexible (yet to defined) option for voting on any topic. Over time we might add more specific vote items like amount of trading fee. To avoid that some stakeholder take benefit of voter apathy and are able to make changes with a very low stake we require a quorum for each vote item. Those quorum values will be defined for each vote item. If the vote item does not reach that limit it will be discarded.
...
*Data structure of the OP_RETURN vote data:*
 ...
 - 1 byte for version (0x01)
 - 20 bytes for hash of encrypted vote data (using Sha256Ripemd160)

e.g. A vote tx would look like that (fee is 5 BSQ):
 - Input 1: 30.00 BSQ (needed for fee payment)
 - Input 2: 0.1 BTC (needed for mining fee)
 - Output 1: 25.00 BSQ (change output used as vote weight)
 - Output 2: 0.09955 BTC (optional BTC change output)
...
*Data structure of the OP_RETURN vote data:*
  ...
  - 32 bytes for decryption key (AES 256 bit)

e.g. A vote reveal tx would look like that:

...
Bisq’s peer-to-peer bitcoin exchange network has been in operation for 18 months now. Exchange volumes have grown along the way, and now is the time for Bisq’s community of contributors to grow along with them. We have designed the Bisq DAO, a _decentralized autonomous organization,_ to support that growth by decentralizing the funding and governance aspects of the project. This document is written to to provide a clear understanding of both Bisq and the Bisq DAO, and then to lay out a specific plan for bootstrapping the Bisq DAO into operation.

*Audience.* This document is intended primarily for Bisq users and contributors, but has been written to be understood by anyone familiar with the Bitcoin ecosystem. If you are interested in working to improve Bisq, or simply interested in where the project is going, read on.

*Organization.* This document is divided into two major parts followed by several appendices:

 - *_<<Part-I>>_* provides a complete overview of and rationale for the Bisq project, the Bisq DAO and BSQ token, and concludes with an analysis of risks inherent to launching the Bisq DAO;

 - *_<<Part-II>>_* presents a set of actions, techniques and principles designed to bootstrap the Bisq DAO into operation while minimizing risk, and goes on to specify a set of exit criteria by which Phase Zero can be considered complete;

 - *_<<Appendix-A>>_* defines the roles individuals play when participating in the Bisq DAO, such as _trader,_ _contributor_ and _stakeholder,_ and defines several categories of high-trust, _bonded contributor roles_ such as _maintainer, operator and administrator;_

 - *_<<Appendix-B>>_* lists the GitHub repositories and other tools used to maintain and operate the Bisq DAO;

 - *_<<Appendix-C>>_* calls out the actions each user role can perform under the Bisq DAO, with specific instructions where appropriate.

*Feedback.* Please ask questions and provide feedback on this document via issues and pull requests in the https://github.com/bisq-network/docs[bisq-network/docs] repository. Alternatively, post questions in the https://forum.bisq.io[Bisq Forum] or chat with us in the https://bisq-slack.herokuapp.com/[Bisq Slack workspace].

*Updates.* This document will remain https://github.com/bisq-network/docs/commits/master/dao/phase-zero.adoc[open to revision] throughout Phase Zero. Watch the bisq-network/docs repository to be notified of changes or subscribe to this document’s dedicated https://github.com/bisq-network/docs/commits/master/dao/overview.adoc.atom[Atom feed].

*Related materials.* A companion slide deck based on this document is available https://docs.google.com/presentation/d/1G5_6Kju2OoItZ5lD0jnskqLdOV2dAJpdB6WOjmq9hRk/edit#[here], a 15-minute interview recapping the Phase Zero plan is available https://www.youtube.com/watch?v=AvXWToT-dcQ[here], and video of a 50-minute talk based on this document will be available soon.
...
Imagine you want to buy bitcoin (BTC) in exchange for US dollars (USD). In Bisq terminology, you are a _buyer_ of BTC looking for a _seller_ of BTC who will accept USD as payment. To complete such a trade using Bisq, you would follow a series of steps similar to these:
 ...
 . You receive the the seller’s bitcoin and the trade is complete
...
Bisq is still small, but has been growing steadily. The monthly USD volume of bitcoin exchanged through the network has doubled roughly every 3½ months since the project went live, from $36K in April 2016 to $438K in September 2017. This growth has been organic, with minimal marketing.

._Bisq global monthly trading volume in USD, April 2016–October 2017_
...
BSQ is Bisq’s own custom implementation of the _colored coin_ concept. From the 25 bitcoin (BTC) that have been donated to the project since its inception in March 2014, we create 2.5 million BSQ tokens, such that each BSQ token is represented on the Bitcoin blockchain by 0.00001000 BTC, or 1000 satoshis.
...
Bisq’s current compensation model has worked well enough so far mainly because the Bisq team has been so small. Bisq’s founders have played most key roles in the system, including that of arbitrator, such that compensating arbitrators with trading fees has, in practice, meant compensating the project’s primary contributors as well. The problem is that this approach breaks down quickly as more contributors get involved, and as explained above, it is critical to the success of the project to grow the number of contributors and to distribute responsibilities amongst them.
...
It is for these reasons that BSQ is needed. As we’ll see in the the sections that follow, the BSQ token has been designed to overcome the challenges described above and to provide a fully decentralized and meritocratic funding and governance model for Bisq.
...
Given current trends, it is important to state explicitly that *BSQ is not associated with an ICO* (initial coin offering), nor will there be any kind of crowdsale or other crowdfunding event. BSQ is a multifaceted _utility token_ being introduced into an already functioning system to make it function even better.
...
Any of these risks could be existential. What’s needed is a conservative and incremental approach to operationalizing the Bisq DAO that accounts for and mitigates these risks. In the next part, we’ll see how the Phase Zero plan is designed to do that.
...
In this section, we present Phase Zero in _negative_ terms, i.e. how the plan is designed to minimize the risks described above and avoid undesirable outcomes. In subsequent sections, we present the plan in _positive_ terms, i.e. the specific set of actions, techniques and principles we will follow to carry out the plan.
...
Completed work for a bounty is, whenever appropriate, submitted as a GitHub pull request. We follow a https://rfc.unprotocols.org/spec:1/C4[C4-based process] in which each repository in the bisq-network GitHub organization has one or more designated #heading=h.z2rcm0qwxl4p[_maintainers]_ responsible for merging or rejecting pull requests. Merged pull requests do not imply or guarantee in any way that the contributor will be compensated in BSQ for their work. Compensation requests are entirely separate from pull requests.

_Status:_ *waiting* for completion of this document to begin work.

=== Define bonded contributor roles

We enumerate and define the _roles_ necessary to operate, maintain and administrate the Bisq project, Bisq network and Bisq DAO, such that responsibilities can be transferred from founders to other reputable contributors. Because each of these of these roles require a degree of trust, filling the role requires putting up a BSQ bond. Initially, founders will use their BSQ stake to bond into these roles, and will then transfer those roles to contributors who (a) wish to take the role over and (b) have earned sufficient BSQ to do so.

_Status:_ *work has been started* in the #heading=h.i0b14vu51gz1[bisq-network/roles repository], where each role has been enumerated in the form of a GitHub issue, but most roles do not yet have a _specification,_ i.e. a detailed description of the responsibilities required to fill that role. The Twitter operator role https://github.com/bisq-network/roles/issues/21[issue] and https://github.com/bisq-network/roles/blob/master/twitter-operator.adoc[specification] are an exception, and provide an early example of what’s to come for the other roles.

=== Establish and refine the fair market value of the BSQ token

Given that BSQ trading is prohibited during Phase Zero, it is not possible for normal market forces to discover a price for the BSQ token. Still, an estimated price for BSQ is required in order to compensate contributors, and we call this estimated price the _fair market value_ of BSQ, borrowing the term from traditional employee stock option valuation processes. This work is one of the first and most important bounty issues, as it is necessary to have a first estimate in hand by the end of October. Beyond our own basic models, we are in communication with a number of cryptoasset analysts to help with this estimation work. As Phase Zero progresses, we expect this estimate and the model behind it to evolve, with the goal of arriving at a maximally credible fair market value for BSQ by the completion of Phase Zero. As mentioned in the risk management section above, this process should significantly mitigate _valuation risk_ when BSQ goes live on mainnet. _Status:_ *work has been started* in the #heading=h.52b7ynoz46ad[BSQ valuation spreadsheet]; further work will continue pending publication of this document and follow-up with analysts.

=== Submit compensation requests and other proposals throughout each month

Status: *in progress.* As of October 2017, all contributions should be captured in one or more compensation requests. See #heading=h.ypi78x1oq459[contributor use cases] for details.

=== Vote on the last 3 days of each month

On the last three days of each month, we vote on contribution requests and other proposals that have been submitted throughout the month. _Status:_ *pending.* The first voting period will begin on October 29th, 2017.

=== Rebase and re-distribute BSQ on the first day of each month

After the monthly voting period is complete and new BSQ is issued, the total amount of BSQ in circulation grows by the amount of that issuance. On the first of every month, we _rebase_ BSQ stake back down to 2.5 MM BSQ, preserving the proportion of stake allotted to each contributor, and issue a new testnet genesis distribution reflecting the stake change. This is done for two reasons: (1) as an additional safeguard to prohibit BSQ trading during Phase Zero, and (2) to accurately reflect the distribution of the 2.5MM BSQ that will be issued on BSQ mainnet upon completion of Phase Zero. _Status:_ *pending;* the first rebasing and re-distribution will occur on November 1st, 2017.

=== Distribute responsibility from founders to bonded contributor roles

Initially, founders use their BSQ stake to assume all bonded contributor roles, reflecting the fact that they do already “have all the keys” and do already carry out most of these duties. As contributors earn sufficient BSQ for bonding, they can approach founders and request to take these roles over. Contributors have a twofold incentive to do so: (1) the BSQ they earn by carrying out the duties of the role, and (2) the interest they earn on their BSQ bond over time. _Status: _*in progress*; initial bonding levels will be set, and founders will be officially bonded into all roles by October 31st, 2017.

=== Grow exchange volume organically

We keep marketing to a minimum during the Phase Zero period, and observe whether our current #heading=h.fayrjpujrybz[growth trend] continues to play out on its own. Rather than focusing on growing our user base, we focus on growing our _contributor_ base. The goal is to foster continued organic growth by making Bisq ever more useful. In any case, we will track the growth trend and adjust course as necessary should it change. _Status:_ *in progress*.
...
As a result, *BSQ utility is limited to _earning_, _voting_ and _bonding_ during Phase Zero.* See the #heading=h.tj841uz3k8o1[risk mitigation] section for rationale.

> _A note to past contributors: When you registered as a past contributor, you provided Bisq’s founders with a mainnet BSQ address. As part of the testnet genesis distribution, we will generate a testnet address on your behalf and correlate it with the mainnet address you’ve already given us._
...
=== Submit compensation requests as GitHub issues in the proposals repository

Eventually, compensation requests will be modeled around Bitcoin transactions. As Phase Zero begins, however, they are modeled as GitHub issues in the #heading=h.jel6upt1tkne[bisq-network/proposals repository].

== General principles

=== Avoid contentious voting

Any conceivable change to the Bisq DAO can be submitted as a proposal, and any proposal can be put to a vote. With that said, the goal should be to vote as infrequently as possible. Where good proposals are submitted, and rough consensus is achieved, no vote is necessary. If rough consensus has not been achieved, then further work is necessary to either improve the proposal or to change minds about it. Voting on a contentious proposal should be seen as a last resort, after all attempts to achieve consensus have failed.
...
Remember: _every contributor_ is free to work on whatever they want, including maintainers who may or may not want to review and merge your pull request if they don’t have any prior context for it, or reason to believe it’s worth spending their time on.
...
The following criteria must be fulfilled in order to exit Phase Zero:
 ...
 - [TBD] Specific volume growth targetor trend
 - [TBD] Specific BSQ fair market value target or trend

See the #heading=h.1h6tl78jsq1d[Roadmap > Completion] for further details.
...
If you +
Read the appendices below. +
Watch the HCPP interview / talk +
Read C4 / Social architecture. +
...
A _trader_ is anyone who uses Bisq to exchange with others. Traders have no specific role to play during Phase Zero, other than continuing to use Bisq as per usual. When Phase Zero is complete, traders will be able to purchase BSQ tokens and use them to pay for Bisq trading fees at a discounted rate.

See #heading=h.mooknes2pmft[post-Phase Zero trader use cases].
...
See #heading=h.ypi78x1oq459[contributor use cases].
...
See #heading=h.i3g1a6rk9xgw[stakeholder use cases].
...
See #heading=h.lpcycbit5ktl[founder use cases].

== Bonded contributor

A _bonded contributor_ is a Bisq contributor who has put up a bond in BSQ in order to take on a _high-trust_ role. As a protection against malfeasance and gross negligence, BSQ bonds may be confiscated (burned) in part or in whole through stakeholder voting. In compensation for making their BSQ illiquid and incurring confiscation risk, bonded contributors earn interest on their BSQ bonds. In compensation for carrying out the specific duties of their role, bonded contributors submit compensation requests no different from any other (non-bonded) contributor.

While there are many specific bonded contributor roles, most fall into one of the categories below.

=== Maintainer

A _maintainer_ is a bonded contributor who is responsible for a given bisq-network GitHub repository, including managing its issues, reviewing pull requests and releasing new versions of the software in that repository.

See #heading=h.pb9gznmaasyl[maintainer use cases].

=== Operator

An _operator_ is a Bisq contributor who runs (“operates”) software necessary to support the Bisq P2P network. Principally, these are Bisq _seed node_ and _price node_ operators, but also include website and DNS operators.

=== Administrator

An _administrator_ is a Bisq contributor who is responsible for a

=== Other roles not listed here

There are more than 30 bonded contributor roles in the Bisq project, including Arbitrator, DNS admin, Reddit moderator and seed node operator. See the #heading=h.i0b14vu51gz1[roles repository and board] for the full list.
...
=== The bisq-network organization

The repositories in the https://github.com/bisq-network[bisq-network GitHub organization] are where changes to Bisq software and documentation are integrated and staged for release. The only individuals who have write access to these repositories are their _maintainers._ Work on bug fixes, features and other improvements is done in contributors’ personal forks of these repositories and changes are then submitted via pull request. Our approach to collaboration is based closely on the https://rfc.unprotocols.org/spec:1/C4[C4 process].
...
=== The roles repository and board
...
=== The BSQ stake and vote tracking spreadsheet

https://docs.google.com/spreadsheets/d/1OOC_gDd4UHfNffs3suU2s2aJfZT_G3w8ZXX-BcfO45s/edit#gid=0[BSQ stake and vote tracking] (Google Sheet, not yet publicly visible)

=== The BSQ valuation spreadsheet
...
=== The BSQ transaction explorer

Testnet BSQ transactions can be explored at https://explorer.bisq.network/testnet[https://explorer.bisq.]https://explorer.bisq.network/testnet[network]https://explorer.bisq.network/testnet[/testnet].
...
Contributors can browse this list to find open bounties they are interested in working on. Note that the list is currently empty. See the #heading=h.jqfqriwhb244[Define bounty issues] section above.
...
Create an issue in the bisq-network/proposals repository, including:

 - The amount you are requesting in BSQ
 - The address that BSQ should be paid to
 - Links to issues and pull requests that you want to be compensated for
 - Commentary on the work that helps explain it, provide context for it, or communicate the value of it.
...
=== vote on proposals (including contributor compensation requests)
...
In accordance with the https://rfc.unprotocols.org/spec:1/C4[C4 process], all contributions to bisq-network repositories should come in the form of pull requests. Repository maintainers should review and comment on pull requests and merge them only if they are correct and well-formed.
...
Each specific role not listed here—e.g. GitHub administrator or Twitter account operator—defines use cases specific to that role. See the bisq-network/roles board for details.

...
We keep marketing to a minimum during the Phase Zero period, and observe whether our current <<growth-rate,growth trend>> continues to play out on its own. Rather than focusing on growing our user base, we focus on growing our _contributor_ base. The goal is to foster continued organic growth by making Bisq ever more useful. In any case, we will track the growth trend and adjust course as necessary should it change. _Status:_ *in progress*.
...
Eventually, compensation requests will be modeled around Bitcoin transactions. As Phase Zero begins, however, they are modeled as GitHub issues in the <<proposals-repo>>.
...
See the <<Completion>> section of the <<Roadmap>> for further details.
...
See <<post-phase-zero-use-cases>> for traders.
...
An _administrator_ is a Bisq contributor who is responsible for a TODO

=== Other roles not listed here

There are more than 30 bonded contributor roles in the Bisq project, including Arbitrator, DNS admin, Reddit moderator and seed node operator. See the <<roles-repo>> for the full list.
...
// TODO: Add a preface here explaining that it not all use cases are fleshed out yet; that the intention is to enumerate first; fully define later

...
You'll build a NodeJS based script that connects to Bisq over HTTP API to get offers and market prices
and sends email notification in case any interesting offer is found.
...
Next we need to filter those offers by price. There are two types of offers, market price based offers and fixed price offers.
They are distinguished by `useMarketBasedPrice` attribute.
In case of market price based offers the filtering criteria is easy, the `marketPriceMargin` must be above 0.1.
In case of fixed price offers we have to fetch market price of BTC in EUR, then calculate price that is 1% less
and finally filter offers which have price less that that calculated price.
...
So first 2 things to do (concurrently and asynchronously) is to fetch offers and market price of BTC from our API.
Then we need to filter those offers and finally send notification.
...
This function filters offers to match our criteria. It returns matching offers and maps them to a bit simpler structure
that contains as little data as needed for `notify` function.
We are using `lodash` library to simplify the filtering.
...
[NOTE]
If you would like to use something else that gmail then you will need a bit different mail transport configuration.
For reference look at https://nodemailer.com/smtp/.
